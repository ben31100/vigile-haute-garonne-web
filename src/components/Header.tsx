...
const Header: React.FC = () => {
  ...
  // Fonction pour naviguer vers une page et remonter en haut
  const navigateAndScrollTop = (path: string) => {
    // Si on est déjà sur la page, simplement remonter en haut
    if (location.pathname === path) {
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      // Sinon, naviguer vers la page (React Router remontera en haut par défaut)
      navigate(path);
    }
  };

  // S'assurer que la page remonte en haut lors du changement de route
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [location.pathname]);
  ...
  return (
    <header className="bg-white shadow-md fixed w-full z-50">
      ...
      {/* Navigation desktop - Modifiée pour utiliser navigateAndScrollTop */}
      ...
      {/* Mobile Menu Button with Sheet - Modifié pour utiliser navigateAndScrollTop */}
